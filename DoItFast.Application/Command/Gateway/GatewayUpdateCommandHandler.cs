...
namespace DoItFast.Application.Command.Gateway
{
    public class GatewayUpdateCommandHandler : ICommandHandler<GatewayUpdateCommand, Response<GatewayResponseDto>>
    {
        ...
        private readonly IRepository<PeripheralDevice> _deviceRepository;
        ...
        public GatewayUpdateCommandHandler(
            ...
            IRepository<PeripheralDevice> deviceRepository,
            IMapper mapper,
        ...
        private void UpdateGateway(Domain.Models.GatewayAggregate.Gateway gateway, GatewayUpdateCommand request)
        {
            ...
            {
                var devicesToRemove = gateway.PeripheralDevices
                    .Where(p => !request.PeripheralDevices.Any(r => r.Id == p.Id))
                    .ToArray();

                var devicesToUpdate = new List<PeripheralDevice>();
                foreach (var device in gateway.PeripheralDevices)
                {
                    var deviceRequest = request.PeripheralDevices.FirstOrDefault(p => p.Id == device.Id);
                    if (deviceRequest != null)
                    {
                        gateway.UpdatePeripheralDevice(device.Id, deviceRequest.Vendor, deviceRequest.PeripheralDeviceStatusId);
                        devicesToUpdate.Add(device);
                    }
                }

                var devicesToAdd = new List<PeripheralDevice>();
                ...
                if (devicesToRemove.Length > 0)
                    _deviceRepository.RemoveRange(devicesToRemove);
